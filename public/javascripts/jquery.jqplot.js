...
(function($) {
   ...
   $.jqplot.ColorGenerator = function(colors) {
    ...
    $.jqplot.MarkerRenderer.prototype.drawjumpCircle = function(x, y, ctx, fill, options) {
        var radius = this.size/1.75;
        var radiusmiddle = this.size/2.25;
        var radiusInner = this.size/2.75;
        var end = 2*Math.PI;
        var point1 = [x, y, radius, 0, end, true];
        var point1a = [x,y,radiusmiddle,0,end,true];
        var point2 = [x, y, radiusInner,0,end,true];
        if (this.shadow) {
            this.shadowRenderer.draw(ctx, point1);
        }
        this.shapeRenderer.draw(ctx, point1, options);
        this.shapeRenderer.draw(ctx, point1a, {color:'#FFFFFF'});
        this.shapeRenderer.draw(ctx, point2,{fill:true}, options);
        // ctx.restore();
    };
        ...
        if (options.show == null || options.show != false) {
            ...
            switch (this.style) {
                ...
                case 'circle':
                    ...
                    break;
                case 'jumpCircle':
                    this.drawjumpCircle(x,y,ctx, true, options);
                    break;
                case 'filledCircle':
